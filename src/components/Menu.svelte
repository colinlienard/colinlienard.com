<script lang="ts">
	...
	import WrenchScrewdriverIcon from 'virtual:icons/heroicons/wrench-screwdriver';
...
<div
	id="menu"
	class="fixed top-12 z-50 w-full max-w-sm rounded-3xl transition-all duration-400 overflow-hidden bg-white/90 backdrop-blur-sm shadow-lg/6 ring ring-black/8 {isOpen
		? 'rounded-b-xl'
		: ''}"
	role="presentation"
...
>
	...
	>
		...
		<kbd
			class="not-sm:hidden mr-1.5 ml-auto flex items-center gap-0.5 rounded-md border border-neutral-200 bg-neutral-100 p-1 font-sans text-sm text-neutral-500 {isKeyDown
				? 'translate-y-0.5'
				: 'border-b-4'}"
		>
	...
	{#if isOpen}
		<div
			class="flex flex-col gap-1.5 p-1.5 pt-0"
			transition:slide={{ duration: TRANSITION_DURATION, easing: quartOut }}
		>
			...
			{@render menuItem(3, translations.stack, WrenchScrewdriverIcon)}
			<hr class="text-black/8 -mx-1.5" />
			...
			<div class="grid grid-cols-3 gap-1.5">
				...
				{@render menuItem(7, translations.system, ComputerDesktopIcon)}
			</div>
		</div>
	{/if}
</div>

{#snippet menuItem(index: number, title: string, Icon?: Component)}
	<button
